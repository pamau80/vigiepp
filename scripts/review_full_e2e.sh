#!/usr/bin/env bash
# Revisión E2E completa — todos los módulos API VigiEPP v44
set -euo pipefail
...
echo "=== VigiEPP FULL API Review v44 ==="

# Health + core
H=$(curl -s "$BASE/api/health")
echo "$H" | $PY -c "import sys,json; d=json.load(sys.stdin); assert d['build']=='v44'" && ok "health v44" || bad "health"
