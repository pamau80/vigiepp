#!/usr/bin/env bash
# Revisión E2E completa — todos los módulos API VigiEPP v45
set -euo pipefail
...
echo "=== VigiEPP FULL API Review v45 ==="

# Health + core
H=$(curl -s "$BASE/api/health")
echo "$H" | $PY -c "import sys,json; d=json.load(sys.stdin); assert d['build']=='v45'" && ok "health v45" || bad "health"
