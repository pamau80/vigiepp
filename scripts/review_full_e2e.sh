#!/usr/bin/env bash
# Revisión E2E completa — todos los módulos API VigiEPP v40
set -euo pipefail
...
echo "=== VigiEPP FULL API Review v40 ==="

# Health + core
H=$(curl -s "$BASE/api/health")
echo "$H" | $PY -c "import sys,json; d=json.load(sys.stdin); assert d['build']=='v40'" && ok "health v40" || bad "health"
