#!/usr/bin/env bash
# Revisión E2E completa — todos los módulos API VigiEPP v46
set -euo pipefail
...
echo "=== VigiEPP FULL API Review v46 ==="

# Health + core
H=$(curl -s "$BASE/api/health")
echo "$H" | $PY -c "import sys,json; d=json.load(sys.stdin); assert d['build']=='v46'" && ok "health v46" || bad "health"
