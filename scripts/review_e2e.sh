...
echo "=== VigiEPP E2E Review v48 ==="

# Health
H=$(curl -s "$BASE/api/health")
echo "$H" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['build']=='v48'; assert 'default_pins' not in d; assert 'privacy' in d" && ok "health v48 sin default_pins" || bad "health"
